...
function isDenyMoving(h, v) {
  return Math.abs(h + v) !== 1;
}
...
function AgentRoadCreator(grid, x = 0, y = 0, r = 0, g = 100, b = 100) {
  ...
  this.tryMove = function tryMove() {
    ...
    if (isDenyMoving(h, v)) return false;

    // if (h === 1 && Math.random() > 0.5) return false;
    ...
    if (isAgent(cellNew)) return false;

    // if (!isEqual(cellNew, this) && !isEqual(cellNew, { r: 0, g: 0, b: 0 })) {
    //   return;
    // }

    if (this.manageAgentCargoCollision(cellOld, cellNew)) return true;
...
function Gamer(matrix) {
  ...
  this.step = function step(grid) {
    ...
    // if (this.agents[0].x === this.agents[1].x && this.agents[0].y === this.agents[1].y) {
    //   debugger;
    // }
