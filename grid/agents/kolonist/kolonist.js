function AgentRoadCreator(grid, x = 0, y = 0, r = 0, g = 100, b = 100) {
  ...
  this.move = function move(h = 0, v = 0) {
    ...
    if (!this.isEqual(cellNew, this) && !this.isEqual(cellNew, { r: 0, g: 0, b: 0 })) return;

    const currentColor = this.grid.matrix.cells[this.x][this.y];

    currentColor.r = this.r;
    currentColor.g = this.g;
    currentColor.b = this.b;
...
function AgentWalker(grid, roadColor, x = 0, y = 0, r = 0, g = 100, b = 100) {
  ...
  this.move = function move(h = 0, v = 0) {
    ...
    if (!(this.isEqual(cellNew, this.roadColor) || this.isEqual(cellNew, this))) return;
...
function Gamer(matrix) {
  ...
  for (let i = 0; i <= agentsCount; i += 1) {
    ...
    const agent = {
      x: Math.floor(Math.random() * this.matrix.columns),
      y: Math.floor(Math.random() * this.matrix.rows),
      r: Math.floor(Math.random() * 255),
      g: Math.floor(Math.random() * 255),
      b: Math.floor(Math.random() * 255),
    };
    // const agent = new AgentRoadCreator(this, x, y, r, g, b);
    // this.agents.push(agent);

    // const agent2 = new AgentRoadCreator(this, x + 20, y, r, b, g);
    // this.agents.push(agent2);

    this.agents.push(
      new AgentRoadCreator(this, { r: agent.r, g: agent.g, b: agent.b }, x, y, 100, 0, 0),
    );
  }
